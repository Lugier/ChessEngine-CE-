...
void NnueEvaluator::recompute_hidden(const int8_t* feat, int32_t* hidden) const {
...
bool NnueEvaluator::try_incremental_update(const Board& b, int32_t* hidden, int8_t* feat) const {
  (void)b;
  (void)hidden;
  (void)feat;
  // Disabled until accumulator-based (pre-activation) incremental update is implemented.
  return false;
}
...
int NnueEvaluator::evaluate(const Board& b) const {
  ...
  bool inc_ok = false;
  if (cache_valid_) {
    for (int j = 0; j < NnueWeights::kHidden; ++j) hidden[j] = cache_hidden_[j];
    for (int i = 0; i < W_.input_dim; ++i) feat[i] = cache_feat_[i];
    inc_ok = try_incremental_update(b, hidden, feat);
  }
  if (!inc_ok) {
    board_to_features(b, feat);
    recompute_hidden(feat, hidden);
